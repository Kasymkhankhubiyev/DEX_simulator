...
class DEXBot:
    def __init__(self, main_menu, window: tk.Tk, data: dict, pool_name: str,
        ...
        self.liq_taker = LTaker({self.pool_name.split("/")[-1].strip(): 10,
                                 ...
                                 quote_price_usd= float(self.pool_data['quote_token_price_usd']),
    ...
    def draw(self) -> None:
        ...
        liq_taker_canvas.grid(row=0, column=1, padx=10, pady=10, rowspan=3, sticky='n')
        ...
        tk.Button(liq_taker_canvas, text='Back to Menu', bg='red', font=title_label_font,
                  command=self._go_to_main_menu).grid(row=liq_taker_canvas_row_num, column=0, 
        ...
        self.pnl_canvas = tk.Canvas(self.window)
        self.pnl_canvas.grid(row=2, column=0, padx=10, pady=10)
