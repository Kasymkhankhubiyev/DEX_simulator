...
class ArbitrageSetting:
    ...
    def draw(self) -> None:
        main_canvas = ctk.CTkFrame(self.window)
        main_canvas.pack(anchor=ctk.CENTER, expand=True)

        ctk.CTkLabel(main_canvas, text='Choose a pool to trade in:',
        ...
        """
        self.lp_type = ctk.CTkComboBox(main_canvas, values=self.pools,
                                        width=400, height=20,
        ...
        for idx, (_name, _value) in enumerate(zip(self._labels_names, _labels_values)):
            ctk.CTkLabel(main_canvas, text=_name, font=Labels_font).grid(row=idx+1, column=0, padx=10, pady=7)
            value_label = ctk.CTkLabel(main_canvas, text=f'{float(_value):.7f}', font=Labels_font)
        ...
        self.back_btn = ctk.CTkButton(main_canvas, text='Back', font=Labels_font,
                                  command=self._back_to_main_menu)
        self.back_btn.grid(row=len(_labels_values)+1, column=0, pady=7, padx=10)

        self.start_btn = ctk.CTkButton(main_canvas, text='Start', font=Labels_font,
